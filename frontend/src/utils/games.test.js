...
// --- Default: both games shown ------------------------------------------------
assert.deepStrictEqual(enabledGames(), ['pokemon', 'mtg']);
assert.ok(showGamePicker(), 'two games means the picker is worth showing');
assert.strictEqual(gameOptions().length, 2);
assert.strictEqual(defaultGameFilter(), '', "'all games' is meaningful with both shown");

// --- Hiding MTG (the exact request in issue #26) -------------------------------
...
assert.deepStrictEqual(enabledGames(), ['pokemon', 'mtg'], 'malformed value falls back to showing everything');
store.set('hidden_games', JSON.stringify(['pokemon', 'mtg', 'gibberish']));
assert.deepStrictEqual(enabledGames(), ['pokemon', 'mtg'], 'hiding every known game is ignored');
store.set('hidden_games', JSON.stringify(['gibberish']));
assert.deepStrictEqual(enabledGames(), ['pokemon', 'mtg'], 'an unknown game name hides nothing');
