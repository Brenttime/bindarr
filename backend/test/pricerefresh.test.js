...
// never swept, which is the whole point of the third case below.
//
// Upstream's list carried pokemon/tcgdex/tcgcsv/lorcana as well. This install is
// MTG-only and the sweep-bookkeeping map kept only the mtg entry, so those names
// have no column to record against and no sweep to run; asserting on them would
// only be asserting a feature this build does not ship. The gate itself is tested
// unchanged.
const PROVIDERS = [
